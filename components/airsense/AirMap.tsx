...
const zoneOverlays: ZoneOverlay[] = [
  { id: 'a-area',     x: 35, y: 25, w: 22, h: 50 },
  { id: 'b-area',     x: 72, y: 22, w: 18, h: 50 },
  { id: 'conference', x: 56, y: 42, w: 14, h: 22 },
  { id: 'discussion', x: 38, y: 76, w: 16, h: 14 },
  { id: 'restroom',   x: 42, y: 8,  w: 10, h: 10 },
];
...
function getScoreLabel(score: number, t: Dictionary) {
  if (score >= 70) return { label: t.airMap.score.FRESH, color: 'text-emerald-400' };
  if (score >= 45) return { label: t.airMap.score.MODERATE, color: 'text-amber-400' };
  return { label: t.airMap.score.POOR, color: 'text-red-400' };
}
...
function ZonePopup({ zone, data, onClose }: PopupProps) {
  const { t } = useI18n();
  const scoreInfo = getScoreLabel(data.airQualityScore, t);
  const zoneName = t.airMap.zoneNames[zone.id];
  return (
    <motion.div
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="text-white font-semibold text-sm">{zoneName}</div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="glass rounded-xl p-2.5">
          <div className="flex items-center gap-1.5 text-slate-400 text-xs mb-1">
            <Wind className="w-3 h-3 text-cyan-400" /> {t.airMap.co2}
          </div>
        ...
        <div className="glass rounded-xl p-2.5">
          <div className="flex items-center gap-1.5 text-slate-400 text-xs mb-1">
            <Thermometer className="w-3 h-3 text-orange-400" /> {t.airMap.temp}
          </div>
        ...
        <div className="glass rounded-xl p-2.5">
          <div className="flex items-center gap-1.5 text-slate-400 text-xs mb-1">
            <Droplets className="w-3 h-3 text-blue-400" /> {t.airMap.humidity}
          </div>
        ...
        <div className="glass rounded-xl p-2.5">
          <div className="flex items-center gap-1.5 text-slate-400 text-xs mb-1">
            <Users className="w-3 h-3 text-violet-400" /> {t.airMap.crowd}
          </div>
      ...
      <div>
        <div className="flex justify-between text-xs mb-1">
          <div className="flex items-center gap-1 text-slate-400">
            <Activity className="w-3 h-3" /> {t.airMap.airQualityScore}
          </div>
...
export default function AirMap({ zoneData }: AirMapProps) {
  ...
  const selectedZone = zoneOverlays.find(z => z.id === selected);
    ...
    <section id="airmap" className="py-24 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-8">
          {[
            { color: 'bg-emerald-400', label: t.airMap.legendFresh },
            { color: 'bg-amber-400', label: t.airMap.legendModerate },
            { color: 'bg-red-400', label: t.airMap.legendPoor },
          ].map(item => (
        ...
        >
          ...
          <div className="relative" style={{ paddingBottom: '60%' }}>
            ...
            {zoneOverlays.map(zone => {
              ...
              const zoneLabel = t.airMap.zoneLabels[zone.id];
              ...
              return (
                <motion.button
                  ...
                  aria-label={t.airMap.viewZoneAria.replace('{zone}', zoneLabel)}
                >
          ...
          <div className="flex flex-wrap items-center gap-4 px-6 py-3 border-t border-white/5 bg-black/20">
            {zoneData.map(z => {
              const sc = getScoreLabel(z.airQualityScore, t);
              return (
                <button
                  key={z.id}
